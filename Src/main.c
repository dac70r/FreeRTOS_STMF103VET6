...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint8_t message_Task01[] = "Hello from Task 1\r\n";
uint8_t message_Task02[] = "Hello from Task 2\r\n";
...
void StartBlink01(void const * argument)
{
  ...
  for(;;)
  {
		HAL_UART_Transmit(&huart1 ,message_Task01, strlen((char*)(message_Task01)),HAL_MAX_DELAY);
    osDelay(1000);
  }
	// if for some reason the program breaks from loop, terminate program
...
void StartBlink02(void const * argument)
{
  ...
  for(;;)
  {
		HAL_UART_Transmit(&huart1 ,message_Task02, strlen((char*)(message_Task02)),HAL_MAX_DELAY);
    osDelay(1000);
  }
	// if for some reason the program breaks from loop, terminate program
